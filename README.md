...
基本設計書（アーキテクチャ図・クラス図/ER図・シーケンス図・フローチャート）を
GitHub Pages で公開しています。

📖 **[LocalChat 基本設計書](https://tkwork-dev.github.io/LocalChat/)**

> 図は Mermaid をオフライン描画しています。`docs/` 配下の HTML をローカルで開いても閲覧できます。

📝 **[環境構築手順書（SETUP.md）](SETUP.md)** — git clone から起動・社内公開までの手順
...
- テキストチャンネル: 作成・閲覧
- メッセージ: 送信・編集・削除・**返信**（元メッセージの直下に表示）・**絵文字リアクション**
- **Markdown記法**: 見出し・太字・斜体・取り消し線・コード/コードブロック（ファイル名表示対応）・引用・リスト・テーブル・リンク（XSS対策済み・依存ライブラリなしの自前実装）
- **図の埋め込み**: Mermaid（ブラウザ内描画）と PlantUML（社内 plantuml.jar による描画、任意設定）
- メンション: `@ユーザー名` による通知
- DM: 1対1 / グループ
- ファイル共有: **ドラッグ＆ドロップ**対応。画像・PDF・Officeファイル等のアップロード/ダウンロード（社内保存）
- **ファイルプレビュー**: HTML / Markdown / PlantUML ファイルはチャット内でインラインプレビュー（ソース表示切替・新しいタブで開く対応）。画像は従来通りサムネイル表示
...
- **コードブロックのファイル名表示**: ` ```ruby:filename.rb ` のように言語の後に `:ファイル名` を付けるとブロック左上にファイル名が表示されます
...
## ファイルのドラッグ＆ドロップとプレビュー

チャットエリアにファイルをドラッグ＆ドロップするとアップロードできます。
ファイルタイプに応じてインラインプレビューが表示されます。

| ファイル種別 | プレビュー表示 |
|---|---|
| 画像（png / jpg / gif 等） | サムネイル画像 |
| HTML（.html / .htm） | iframe によるレンダリング表示 |
| Markdown（.md） | Markdown→HTMLレンダリング表示 |
| PlantUML（.puml） | SVG図として描画 |
| その他 | 📎 リンク表示 |

すべてのプレビューには **`</>` ソース表示切替**ボタンと **`↗` 新しいタブで開く**リンクが付きます。
プレビュー枠の横幅はドラッグでリサイズ可能です。

## タイムゾーン設定

メッセージの投稿時刻は `.env` の `TZ_OFFSET_HOURS` で制御します。

```
# UTCからのオフセット時間。日本なら 9、UTCなら 0。
TZ_OFFSET_HOURS=9
```

既定値は `9`（JST）です。海外拠点で利用する場合はその地域のオフセットに変更してください。
...
本アプリは**社内LAN（閉域網）での自社内利用**を想定して設計しており、その場合は電気通信事業法上の届出・登録は不要と考えられます。

ただし、**社外の第三者と通信する形態**（グループ会社・取引先への提供、インターネットへの公開、SaaS としての提供など）で利用する場合は、**電気通信事業に該当し、事前の届出または登録が必要**になる可能性があります。

| 利用形態 | 該当性 | 手続 |
|----------|--------|------|
| 自社の役職員のみで利用（社内閉域網） | 非該当の可能性が高い | 原則不要 |
| グループ会社・取引先など「他人」へ提供 | 該当しうる | 届出の要否を要確認 |
| 不特定多数へSaaS等として提供 | 該当する可能性が高い | 登録または届出が必要 |

外部との通信を伴う利用形態に変更する場合は、**利用開始前に総務省（管轄の総合通信局）へ確認**し、必要な手続を行ってください。

> 詳細な法的検討は [law.md](law.md) を参照してください。なお、本記載は一般的な情報提供であり法的助言ではありません。最終判断は専門家にご相談ください。
